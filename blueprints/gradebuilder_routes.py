...
def gb_get_data():
    ...
    try:
        with get_db_connection().cursor() as cursor:
            ...
            # Debug: Log classes data to verify class_type is included
            logger.info(f"API /gradebuilder/data returning {len(classes)} classes")
            for cls in classes:
                logger.info(f"  Class {cls.get('class_id')}: type={cls.get('class_type')}")
