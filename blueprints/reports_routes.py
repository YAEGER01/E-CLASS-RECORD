...
def generate_grade_sheet_report(class_id):
    ...
    try:
        with get_db_connection().cursor() as cursor:
            ...
            course = class_row[1] if isinstance(class_row, tuple) else class_row.get('course')
            subject = class_row[2] if isinstance(class_row, tuple) else class_row.get('subject')
            section = class_row[3] if isinstance(class_row, tuple) else class_row.get('section')
            year = class_row[7] if isinstance(class_row, tuple) else class_row.get('year')
            semester = class_row[8] if isinstance(class_row, tuple) else class_row.get('semester')
            track = class_row[9] if isinstance(class_row, tuple) else class_row.get('track')
            subject_code = class_row[10] if isinstance(class_row, tuple) else class_row.get('subject_code')
            units = class_row[11] if isinstance(class_row, tuple) else class_row.get('units')
            ...
            class_info = {
                'class_code': class_row[0] if isinstance(class_row, tuple) else class_row.get('class_code'),
                'course': course,
                'subject': subject,
                'subject_code': subject_code,
                'units': units,
                'section': section,
                'schedule': class_row[4] if isinstance(class_row, tuple) else class_row.get('schedule'),
                'class_type': class_row[5] if isinstance(class_row, tuple) else class_row.get('class_type'),
            ...
            # Get instructor name
            instructor_id = class_row[6] if isinstance(class_row, tuple) else class_row.get('instructor_id')
            cursor.execute("""
            ...
            if instructor_row:
                inst_first = instructor_row[0] if isinstance(instructor_row, tuple) else instructor_row.get('first_name')
                inst_last = instructor_row[1] if isinstance(instructor_row, tuple) else instructor_row.get('last_name')
                class_info['instructor_name'] = f"{inst_first} {inst_last}".strip().upper() if inst_first and inst_last else 'INSTRUCTOR'
            ...
            if sig_row:
                signatures = {
                    'submitted_by_name': sig_row[0] if isinstance(sig_row, tuple) else sig_row.get('submitted_by_name') or class_info['instructor_name'],
                    'submitted_by_title': sig_row[1] if isinstance(sig_row, tuple) else sig_row.get('submitted_by_title') or 'Assistant Professor IV',
                    'checked_by_name': sig_row[2] if isinstance(sig_row, tuple) else sig_row.get('checked_by_name') or '',
                    'checked_by_title': sig_row[3] if isinstance(sig_row, tuple) else sig_row.get('checked_by_title') or 'Chair, BSIT',
                    'countersigned_by_name': sig_row[4] if isinstance(sig_row, tuple) else sig_row.get('countersigned_by_name') or '',
                    'countersigned_by_title': sig_row[5] if isinstance(sig_row, tuple) else sig_row.get('countersigned_by_title') or 'College Secretary',
                    'noted_by_name': sig_row[6] if isinstance(sig_row, tuple) else sig_row.get('noted_by_name') or '',
                    'noted_by_title': sig_row[7] if isinstance(sig_row, tuple) else sig_row.get('noted_by_title') or 'Dean, CCSICT'
                }
            else:
        ...
        for row in student_rows:
            if isinstance(row, tuple):
                student_id, last_name, first_name, middle_name, final_grade, equivalent, school_id, remarks = row
            else:
            ...
            if not remarks:
                ...
                if equivalent == "INC" or "NPE" in remarks or "NME" in remarks or "NFE" in remarks or "NO PROJECT" in remarks:
                    incomplete_count += 1
                elif remarks == "PASSED":
                    passed_count += 1
                elif remarks == "FAILED":
        ...
        return render_template('grade_sheet_report.html',
...
@reports_bp.route("/api/reports/grade-sheet/<int:class_id>/signatures", methods=["GET", "POST"])
@login_required
def manage_grade_sheet_signatures(class_id):
    ...
    conn = pymysql.connect(
        ...
        password=os.getenv("LOCAL_DB_PASSWORD", "loleris1234"),
        database=os.getenv("LOCAL_DB_NAME", "e_class_record"),
    ...
    try:
        ...
        if request.method == "GET":
                ...
                if sig_row:
                    signatures = {
                        'submitted_by_name': sig_row[0] if isinstance(sig_row, tuple) else sig_row.get('submitted_by_name'),
                        'submitted_by_title': sig_row[1] if isinstance(sig_row, tuple) else sig_row.get('submitted_by_title'),
                        'checked_by_name': sig_row[2] if isinstance(sig_row, tuple) else sig_row.get('checked_by_name'),
                        'checked_by_title': sig_row[3] if isinstance(sig_row, tuple) else sig_row.get('checked_by_title'),
                        'countersigned_by_name': sig_row[4] if isinstance(sig_row, tuple) else sig_row.get('countersigned_by_name'),
                        'countersigned_by_title': sig_row[5] if isinstance(sig_row, tuple) else sig_row.get('countersigned_by_title'),
                        'noted_by_name': sig_row[6] if isinstance(sig_row, tuple) else sig_row.get('noted_by_name'),
                        'noted_by_title': sig_row[7] if isinstance(sig_row, tuple) else sig_row.get('noted_by_title')
                    }
                else:
                    # Get instructor name for default
                    instructor_id = class_row[0] if isinstance(class_row, tuple) else class_row.get('instructor_id')
                    cursor.execute("""
                        SELECT pi.first_name, pi.last_name
                        FROM instructors i
                        LEFT JOIN personal_info pi ON i.personal_info_id = pi.id
                    ...
                    if instructor_row:
                        inst_first = instructor_row[0] if isinstance(instructor_row, tuple) else instructor_row.get('first_name')
                        inst_last = instructor_row[1] if isinstance(instructor_row, tuple) else instructor_row.get('last_name')
                        instructor_name = f"{inst_first} {inst_last}".strip().upper() if inst_first and inst_last else ''
            ...
            return jsonify({"success": True, "message": "Signatures updated successfully"}), 200
