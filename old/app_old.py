...
def student_login():
    ...
    if url_username and url_password:
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM users WHERE school_id = %s", (url_username,)
                )
                user = cursor.fetchone()
        except Exception as e:
            logger.error(f"Database error during admin user lookup: {str(e)}")
            return render_template(
                "adminlogin.html", error="An error occurred. Please try again."
            )

        if (
            user
            and check_password_hash(user["password_hash"], url_password)
            and user["role"] == url_role
        ):
            # Login successful
            session["user_id"] = user.id
            session["school_id"] = user.school_id
