...
document.addEventListener('DOMContentLoaded', () => {
  ...
  */
  ...
  // Function to show/hide laboratory column based on class type
  function updateLabColumnVisibility(classId) {
    const labColumn = document.getElementById('lab-column');
    const classTypeIndicator = document.getElementById('class-type-indicator');
    const classTypeText = document.getElementById('class-type-text');
    
    if (!labColumn) return;
    
    // If no class selected, hide lab by default
    if (!classId) {
      labColumn.style.display = 'none';
      if (labList) labList.innerHTML = '';
      if (classTypeIndicator) classTypeIndicator.style.display = 'none';
      updateSums();
      return;
    }
    
    // Find the selected class
    const selectedClass = (window._classes || []).find(c => String(c.class_id) === String(classId));
    const classType = selectedClass ? selectedClass.class_type : null;
    
    // Debug logging
    console.log('Selected Class ID:', classId);
    console.log('Selected Class:', selectedClass);
    console.log('Class Type:', classType);
    console.log('Available Classes:', window._classes);
    
    // Update the class type indicator
    if (classTypeIndicator && classTypeText) {
      if (classType) {
        let displayText = '';
        let bgColor = '';
        let textColor = '';
        
        if (classType === 'MAJOR_LAB') {
          displayText = '📚 MAJOR (with Laboratory) - 40% Lab + 60% Lecture';
          bgColor = '#dbeafe';
          textColor = '#1e40af';
          labColumn.style.display = '';
          console.log('✅ Showing LABORATORY column for MAJOR_LAB');
        } else if (classType === 'MAJOR') {
          displayText = '📖 MAJOR (Lecture Only) - 100% Lecture';
          bgColor = '#fef3c7';
          textColor = '#92400e';
          labColumn.style.display = 'none';
          if (labList) labList.innerHTML = '';
          console.log('✅ Hiding LABORATORY column for MAJOR');
        } else if (classType === 'MINOR') {
          displayText = '📝 MINOR (Lecture Only) - 100% Lecture';
          bgColor = '#d1fae5';
          textColor = '#065f46';
          labColumn.style.display = 'none';
          if (labList) labList.innerHTML = '';
          console.log('✅ Hiding LABORATORY column for MINOR');
        } else {
          displayText = '⚠️ Unknown Class Type';
          bgColor = '#fee2e2';
          textColor = '#991b1b';
          labColumn.style.display = 'none';
          if (labList) labList.innerHTML = '';
        ...
        classTypeText.textContent = displayText;
        classTypeIndicator.style.backgroundColor = bgColor;
        classTypeIndicator.style.color = textColor;
        classTypeIndicator.style.display = 'block';
      } else {
        classTypeIndicator.style.display = 'none';
        labColumn.style.display = 'none';
        if (labList) labList.innerHTML = '';
        console.log('⚠️ No class type found');
      }
    } else {
      // Fallback if indicator elements don't exist
      if (classType === 'MAJOR_LAB') {
        labColumn.style.display = '';
        console.log('✅ Fallback: Showing LABORATORY column for MAJOR_LAB');
      } else {
        labColumn.style.display = 'none';
        if (labList) labList.innerHTML = '';
        console.log('✅ Fallback: Hiding LABORATORY column');
      }
    }
  ...
  if (classSelect) {
    classSelect.addEventListener('change', (e) => {
      updateLabColumnVisibility(e.target.value);
    });
  }

  // Initialize lab column visibility (hide by default until class is selected)
  updateLabColumnVisibility(null);
  ...
  window._instructor = data.instructor || null;
      ...
      // DEBUG: Log all classes with their types
      console.log('📊 Loaded classes from API:', window._classes);
      window._classes.forEach(c => {
        console.log(`  - Class ${c.class_id}: ${c.course} ${c.section}, type=${c.class_type}`);
      });
      
      // Check if class_id is provided from server (from Grade Entry button)
      let urlClassId = window._preselectClassId || null;
      console.log('🎯 Preselected class_id from server:', urlClassId);
      ...
      if (classSelect) {
        ...
        // If class_id was provided, auto-select it
        if (urlClassId) {
          classSelect.value = String(urlClassId);
          console.log('✅ Auto-selecting class:', urlClassId, 'Dropdown value now:', classSelect.value);
          
          // Trigger immediate class type detection
          setTimeout(() => {
            console.log('🔍 About to detect class type for:', classSelect.value);
            updateLabColumnVisibility(classSelect.value);
          }, 200);
        }
      }
  ...
  if (newStructureBtn) newStructureBtn.addEventListener('click', async (ev) => {
    ...
    if (window.Swal) {
      ...
      if (formValues) {
        ...
        // Update lab column visibility based on selected class
        console.log('Auto-detecting class type for new structure with class:', formValues.classId);
        updateLabColumnVisibility(formValues.classId);
        if (labList) labList.innerHTML = '';
  ...
  window._pendingClassId = s.class_id;
    updateSaveEnabled();
    // Update lab column visibility based on class type - ensure classes are loaded
    setTimeout(() => {
      console.log('Auto-detecting class type when loading structure for class:', s.class_id);
      updateLabColumnVisibility(s.class_id);
    }, 50);
