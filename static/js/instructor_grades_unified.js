...
(function() {
  ...
  class GradeManagementSystem {
    constructor() {
      ...
      this.AUTO_SAVE_DELAY = 2000; // Auto-save after 2 seconds of inactivity
    }
      ...
      try {
        ...
        const studentsPayload = this.getStudentRows().map(tr => {
          ...
          tr.querySelectorAll('input.score').forEach(inp => {
            ...
            const v = inp.value;
            // Keep blank values as null instead of skipping them
            if (v === '') {
              scores[String(aid)] = null;
            } else {
        ...
        // Use summaries from backend for all class types (MINOR and MAJOR)
        if (out.summaries && typeof out.summaries === 'object') {
          this.latestSummaries = out.summaries;
        } else if (out.results) {
          // For MAJOR classes, extract summaries from results._summary
          this.latestSummaries = {};
          Object.entries(out.results).forEach(([sid, data]) => {
            if (data._summary) {
              this.latestSummaries[sid] = data._summary;
            }
          });
        } else {
          this.latestSummaries = {};
     ...
     */
    scheduleAutoSave() {
      // Don't auto-save in limited (student) view
      if (!this.AUTO_SAVE_ENABLED || this.limitedView || this.inputsLocked) return;
    ...
    async autoSaveChanges() {
      // Don't auto-save if already saving, in limited view, or inputs are locked
      if (this.isSaving || this.limitedView || this.inputsLocked || !this.dirty.size) return;
      
      const classId = this.classId || 0;
      if (!classId) return;

      this.isSaving = true;
      const originalStatus = this.status ? this.status.textContent : '';
      ...
      try {
        ...
        if (this.status) this.status.textContent = 'Auto-saving...';
        
        const headers = { 'Content-Type': 'application/json' };
        if (this.csrfToken) headers['X-CSRFToken'] = this.csrfToken;
        
        const res = await fetch(`/classes/${classId}/save-snapshot`, { 
          method: 'POST', 
          headers, 
        ...
        if (res.ok) {
          // Clear dirty state and persisted drafts on successful save
          this.dirty.clear();
          this.persistClear();
          if (this.status) this.status.textContent = 'All changes saved';
          
          // Request fresh compute values from server to update UI
          try { this.computeServerForAll(); } catch(_) {}
        } else {
          if (this.status) this.status.textContent = out.error || 'Auto-save failed';
          console.error('Auto-save failed:', out.error || out.message);
        }
      } catch (err) {
        console.error('Auto-save error:', err);
        if (this.status) this.status.textContent = 'Auto-save failed - network error';
      } finally {
        this.isSaving = false;
      }
    }
    ...
    applyClassTypeDecorations() {
      ...
      const labHdr = document.querySelector('table.final-grade-table thead th.laboratory-header');
      const labCells = document.querySelectorAll('table.final-grade-table tbody td.laboratory-grade');
      
      if (!rawHdr || !totalHdr) return;
      
      // Check if this class has laboratory assessments
      const hasLabCategory = Object.values(this.SUB_GROUPS).some(g => g.category.toUpperCase() === 'LABORATORY');
      
      if (this.isMinor) {
        rawHdr.textContent = 'INITIAL GRADE';
        totalHdr.textContent = 'FINAL GRADE';
        // Hide laboratory column for MINOR
        if (labHdr) labHdr.style.display = 'none';
        labCells.forEach(cell => cell.style.display = 'none');
        // Hide INITIAL GRADE column in GRADE CALCULATION for MINOR without lab
        this.hideInitialGradeColumnForNoLab();
      } else {
        rawHdr.textContent = 'RAW GRADE';
        totalHdr.textContent = 'TOTAL GRADE';
        
        // For MAJOR_LAB, ALWAYS show laboratory column (even if no assessments yet)
        if (this.classType === 'MAJOR_LAB') {
          if (labHdr) labHdr.style.display = '';
          labCells.forEach(cell => cell.style.display = '');
        } else if (hasLabCategory) {
          // MAJOR with LAB assessments: Show laboratory column
          if (labHdr) labHdr.style.display = '';
          labCells.forEach(cell => cell.style.display = '');
        } else {
          // MAJOR without LAB: Hide laboratory column
          if (labHdr) labHdr.style.display = 'none';
          labCells.forEach(cell => cell.style.display = 'none');
          // Hide INITIAL GRADE column in GRADE CALCULATION for MAJOR without lab
          this.hideInitialGradeColumnForNoLab();
        }
      }
    }

    hideInitialGradeColumnForNoLab() {
      // For MAJOR_LAB classes, keep BOTH INITIAL and FINAL grades for BOTH categories visible
      if (this.classType === 'MAJOR_LAB') {
        return; // Exit early - keep all columns visible
      }
      
      // For MINOR and MAJOR without lab: Hide INITIAL GRADE column for LECTURE (keep only FINAL GRADE)
      const initialGradeHeaders = document.querySelectorAll('th.rating-col[data-category="LECTURE"]');
      initialGradeHeaders.forEach(h => h.style.display = 'none');
      
      // Hide INITIAL GRADE cells for LECTURE
      const initialGradeCells = document.querySelectorAll('td.rating-col[data-category="LECTURE"]');
      initialGradeCells.forEach(c => c.style.display = 'none');
      
      // Adjust GRADE CALCULATION header colspan from 2 to 1 for LECTURE
      const gradeCalcHeaders = document.querySelectorAll('th.grade-calc-header[data-category="lecture"]');
      gradeCalcHeaders.forEach(h => h.setAttribute('colspan', '1'));
      
      // Check if LABORATORY category exists in SUB_GROUPS
      const hasLabCategory = Object.values(this.SUB_GROUPS).some(g => g.category.toUpperCase() === 'LABORATORY');
      
      // If no lab category, force hide all LABORATORY elements
      if (!hasLabCategory) {
        // Hide all LABORATORY subcategory headers
        const labSubheads = document.querySelectorAll('th.subcell.subcat-head[data-category="laboratory"]');
        labSubheads.forEach(h => h.style.display = 'none');
        
        // Hide LABORATORY GRADE CALCULATION header
        const labGradeCalcHeaders = document.querySelectorAll('th.grade-calc-header[data-category="laboratory"]');
        labGradeCalcHeaders.forEach(h => h.style.display = 'none');
        
        // Hide LABORATORY INITIAL GRADE header
        const labInitialHeaders = document.querySelectorAll('th.rating-col[data-category="LABORATORY"]');
        labInitialHeaders.forEach(h => h.style.display = 'none');
        
        // Hide LABORATORY FINAL GRADE header
        const labFinalHeaders = document.querySelectorAll('th.final-col[data-category="LABORATORY"]');
        labFinalHeaders.forEach(h => h.style.display = 'none');
        
        // Hide LABORATORY INITIAL GRADE cells
        const labInitialCells = document.querySelectorAll('td.rating-col[data-category="LABORATORY"]');
        labInitialCells.forEach(c => c.style.display = 'none');
        
        // Hide LABORATORY FINAL GRADE cells
        const labFinalCells = document.querySelectorAll('td.final-col[data-category="LABORATORY"]');
        labFinalCells.forEach(c => c.style.display = 'none');
      }
    }
    ...
    setupEventListeners() {
      ...
      this.tbody.addEventListener('input', (e) => {
        ...
        // Update status message with auto-save info
        const autoSaveMsg = this.AUTO_SAVE_ENABLED && !this.limitedView && !this.inputsLocked ? ' (auto-saving...)' : '';
        this.status.textContent = `${this.dirty.size} change(s)${autoSaveMsg}`;
      ...
      document.addEventListener('change', async (e) => {
        ...
        if (!checkbox.matches('input.dropped-checkbox')) {
          return;
        }
        
        console.log('Dropped checkbox clicked!');
        ...
        const isDropping = checkbox.checked;
        
        console.log('Student ID:', studentId, 'Is dropping:', isDropping);
        
        // Show confirmation dialog
        try {
          const title = isDropping ? 'Mark Student as DROPPED?' : 'Restore Student?';
          const text = isDropping 
            ? 'This will mark the student as DROPPED. The grade will be set to DRP across the system.'
            : 'This will restore the student. Their grades will be recalculated normally.';
          const confirmText = isDropping ? 'Drop Student' : 'Restore';
          
          console.log('Showing Swal confirmation...');
          ...
          const result = await Swal.fire({
            ...
            confirmButtonColor: isDropping ? '#dc2626' : '#059669',
          ...
          if (!result.isConfirmed) {
            ...
            console.log('User cancelled, reverting checkbox');
            checkbox.checked = !isDropping;
            return;
          }
          ...
          const csrfToken = pageData ? pageData.getAttribute('data-csrf-token') : '';
          
          console.log('CSRF Token:', csrfToken);
          console.log('Class ID:', self.classId);
          
          // Save dropped status to database
          const url = `/api/classes/${self.classId}/students/${studentId}/dropped`;
          ...
          if (!response.ok) {
            ...
            throw new Error('Failed to update dropped status');
          }
          
          const responseData = await response.json();
          console.log('API response:', responseData);
          
          // Update UI - need to find the row in the main grade table too
          const mainRow = self.tbody.querySelector(`tr[data-student-id="${studentId}"]`);
          if (mainRow) {
            self.updateFinalGradeForRow(mainRow);
            // Disable/enable all input fields in the main row
            self.toggleStudentInputs(mainRow, isDropping);
          }
          
          // Also update the final grade row
          self.updateFinalGradeForRow(tr);
          
          if (self.status) {
            self.status.textContent = isDropping ? 'Student marked as DROPPED' : 'Student restored';
          }
          
          // Show success message
          Swal.fire({
            title: 'Success!',
            text: isDropping ? 'Student has been marked as DROPPED.' : 'Student has been restored.',
            icon: 'success',
            timer: 2000,
            showConfirmButton: false
          });
          
          // If student was dropped, also trigger server compute to update released grades if needed
          if (self.AUTO_SAVE_ENABLED && !self.limitedView) {
            self.scheduleAutoSave();
          }
        } catch (err) {
          console.error('Failed to update dropped status:', err);
          // Revert checkbox on error
          checkbox.checked = !isDropping;
          this.status.textContent = 'Failed to update dropped status';
          
          // Show error message
          Swal.fire({
            title: 'Error',
            text: 'Failed to update dropped status. Please try again.',
            icon: 'error',
      ...
      document.addEventListener('click', async (e) => {
        ...
        if (btnSubmit) {
          e.preventDefault();
          // Cancel any pending auto-save since we're manually submitting
          if (this.autoSaveTimer) {
            clearTimeout(this.autoSaveTimer);
            this.autoSaveTimer = null;
          }
    ...
    async handleAdd(btn) {
      ...
      try {
        ...
        if (created.length && !failed.length) {
          await swalAlert('success', 'Assessments created', `${created.length} assessment(s) added under ${subcategory}. Page will refresh.`);
          // Auto-reload after successful creation
          setTimeout(() => window.location.reload(), 1000);
          return;
        }
        if (created.length && failed.length) {
          await swalAlert('warning', 'Partial success', `${created.length} created, ${failed.length} failed. Page will refresh.`);
          // Auto-reload after partial success
          setTimeout(() => window.location.reload(), 1000);
          return;
        }
        // none created
    ...
    async handleEditAssessment(editBtn) {
      ...
      if (!SwalLib) {
        ...
        try {
          ...
          const th = document.querySelector(`th.assess-col[data-assessment-id="${assessmentId}"]`);
          if (th) {
            th.setAttribute('data-max', String(newMax));
            th.setAttribute('title', `Max ${newMax}`);
            // Update visible max score text
            const maxDiv = th.querySelector('div.max');
            if (maxDiv) {
              const editBtn = maxDiv.querySelector('button[data-action="edit-assessment"]');
              if (editBtn) {
                editBtn.setAttribute('data-max', String(newMax));
                maxDiv.childNodes[0].textContent = `/ ${newMax} `;
              } else {
                maxDiv.textContent = `/ ${newMax}`;
              }
            }
          }
          document.querySelectorAll(`input.score[data-assessment="${assessmentId}"]`).forEach(inp => inp.setAttribute('max', String(newMax)));
          // Refresh structure and trigger single recompute
          try { this.GROUPED_CACHE = null; this.buildSubGroups(); } catch(_) {}
          if (this.serverComputeTimer) clearTimeout(this.serverComputeTimer);
          this.serverComputeTimer = setTimeout(() => { this.computeServerForAll(); this.serverComputeTimer = null; }, 100);
          window.alert('Max score updated');
      ...
      if (res.isDenied) {
        ...
        try {
          ...
          await swalAlert('success', 'Deleted', 'Assessment was deleted. The page will refresh.');
      ...
      if (res.isConfirmed) {
        ...
        try {
          ...
          // Update the max value in the DOM without reloading
          const th = document.querySelector(`th.assess-col[data-assessment-id="${assessmentId}"]`);
          if (th) {
            th.setAttribute('data-max', String(newMax));
            th.setAttribute('title', `Max ${newMax}`);
            // Update visible max score text in the header
            const maxDiv = th.querySelector('div.max');
            if (maxDiv) {
              const editBtn = maxDiv.querySelector('button[data-action="edit-assessment"]');
              if (editBtn) {
                // Update the button's data-max attribute
                editBtn.setAttribute('data-max', String(newMax));
                // Update the text node before the button (the "/ X" part)
                // We need to update only the text node, not the button
                const textNode = Array.from(maxDiv.childNodes).find(node => node.nodeType === Node.TEXT_NODE && node.textContent.includes('/'));
                if (textNode) {
                  textNode.textContent = `/ ${newMax} `;
                } else {
                  // Fallback: rebuild the text
                  maxDiv.childNodes[0].textContent = `/ ${newMax} `;
                }
              } else {
                maxDiv.textContent = `/ ${newMax}`;
          ...
          const inputs = document.querySelectorAll(`input.score[data-assessment="${assessmentId}"]`);
          inputs.forEach(inp => inp.setAttribute('max', String(newMax)));
          // Refresh structure (clear cache so next computation uses new max)
          try { this.GROUPED_CACHE = null; this.buildSubGroups(); } catch(_) {}
          // Trigger a single server recompute with the new max
          if (this.serverComputeTimer) clearTimeout(this.serverComputeTimer);
          this.serverComputeTimer = setTimeout(() => { this.computeServerForAll(); this.serverComputeTimer = null; }, 100);
      ...
      if (this.dirty && this.dirty.size) {
        ...
        for (const v of this.dirty.values()) {
          scores.push({
            ...
            assessment_id: v.assessment_id,
            class_id: v.class_id || classId,
            score: v.score,  // Keep null as null, don't convert to 0
          });
        }
      } else {
        ...
        document.querySelectorAll('input.score').forEach(inp => {
          ...
          if (!sid || !aid) return;
          const val = inp.value;
          const sv = val === '' ? null : parseFloat(val);  // Keep blank as null
    ...
    applyLockState(locked) {
      ...
      // Cancel any pending auto-save when locking
      if (locked && this.autoSaveTimer) {
        clearTimeout(this.autoSaveTimer);
        this.autoSaveTimer = null;
      }
    ...
    recomputeCategoryRatings(tr) {
      ...
      // Check if there's laboratory category
      const hasLabCategory = cats.some(c => c.toUpperCase() === 'LABORATORY');
      
      // For each category, sum all % of REQUIREMENT values
      cats.forEach(cat => {
        let sum = 0;
        let any = false;
        ...
        // Update the INITIAL GRADE cell for this category (sum of % of requirements)
        const ratingCell = tr.querySelector(`td.computed.rating-col[data-category="${cat}"]`);
        if (ratingCell) {
          ratingCell.textContent = any ? this.fmt(sum, 2) : '';
        }
        
        // Update the FINAL GRADE cell for this category
        const finalCell = tr.querySelector(`td.computed.final-col[data-category="${cat}"]`);
        if (finalCell) {
          if (any) {
            ...
            if (hasLabCategory) {
              // MAJOR with LAB: Apply 0.6 for LECTURE, 0.4 for LABORATORY
              if (catUpper === 'LECTURE') {
                weighted = sum * 0.6;
              } else if (catUpper === 'LABORATORY') {
                weighted = sum * 0.4;
              } else {
                weighted = sum;
              }
            } else {
              // MINOR or MAJOR without LAB: Don't apply multiplier, just show the sum
              weighted = sum;
            }
            
            finalCell.textContent = this.fmt(weighted, 2);
          } else {
    ...
    updateFinalGradeForRow(tr) {
      ...
      // Get FINAL GRADE values from per-category FINAL GRADE columns (not INITIAL GRADE)
      const lectureFinalTd = tr.querySelector('td.computed.final-col[data-category="LECTURE"]');
      const labFinalTd = tr.querySelector('td.computed.final-col[data-category="LABORATORY"]');
      const lectureFinal = lectureFinalTd ? parseFloat((lectureFinalTd.textContent||'').trim()) : NaN;
      const labFinal = labFinalTd ? parseFloat((labFinalTd.textContent||'').trim()) : NaN;
      
      const finalRow = document.querySelector(`table.final-grade-table tbody tr[data-student-id="${sid}"]`);
      if (!finalRow) return;
      const lectureCell = finalRow.querySelector('td.lecture-grade');
      const laboratoryCell = finalRow.querySelector('td.laboratory-grade');
      ...
      if (!hasAnyInputValue) {
        if (lectureCell) lectureCell.textContent = '';
        if (laboratoryCell) laboratoryCell.textContent = '';
      ...
      const lecVal = isNaN(lectureFinal) ? 0 : lectureFinal;
      const labVal = isNaN(labFinal) ? 0 : labFinal;
      
      // Update LECTURE and LABORATORY cells in final grade table with FINAL GRADE values
      if (lectureCell) {
        lectureCell.textContent = !isNaN(lectureFinal) ? lectureFinal.toFixed(2) : '';
      }
      if (laboratoryCell) {
        laboratoryCell.textContent = !isNaN(labFinal) ? labFinal.toFixed(2) : '';
      }
      
      const summary = (this.latestSummaries && this.latestSummaries[sid]) ? this.latestSummaries[sid] : null;
      
      // Use backend summary if available (preferred)
      let rawValue;
      let totalGrade;
      
      if (summary) {
        // Backend provides the correct values
        rawValue = typeof summary.lecture === 'number' ? summary.lecture : (typeof summary.initial_grade === 'number' ? summary.initial_grade : 0);
        totalGrade = typeof summary.final_grade === 'number' ? summary.final_grade : 0;
      } else {
        // Fallback: compute locally
        const hasLab = !isNaN(labVal) && labVal > 0;
        
        if (hasLab) {
          // MAJOR with LAB: Add lecture and laboratory, then apply transformation
          rawValue = Math.round((lecVal + labVal) * 100) / 100;
          totalGrade = Math.round((rawValue * 0.625 + 37.5) * 100) / 100;
        } else if (this.isMinor) {
          // MINOR: Use lecture only, apply transformation
          rawValue = lecVal;
          totalGrade = Math.round((lecVal * 0.5 + 50) * 100) / 100;
        } else {
          // MAJOR without LAB: Use lecture only, apply transformation
          rawValue = lecVal;
          totalGrade = Math.round((lecVal * 0.625 + 37.5) * 100) / 100;
        }
      }
      ...
      // Check for missing required assessments FIRST before computing grade
      let missingAssessments = [];
      let hasAnyAssessments = false;
      
      // Check if student is marked as DROPPED
      const droppedCheckbox = tr.querySelector('.dropped-checkbox');
      const isDropped = droppedCheckbox ? droppedCheckbox.checked : false;
      
      if (remarksCell && !isDropped) {
        const heads = Array.from(document.querySelectorAll('th.assess-col'));
        const subcats = {};
        heads.forEach(h => {
        ...
        // Check each subcategory for missing assessments
        for (const [subName, ids] of Object.entries(subcats)) {
          ...
          if (anyMissing) {
            missingAssessments.push(subName);
          }
        }
      }

      // Compute GRADE MARK from TOTAL GRADE using ISU official grading scale
      // If missing required assessment or dropped, set accordingly
      if (gradeMarkCell) {
        let gm = '';
        
        // If student is DROPPED
        if (isDropped) {
          gm = 'DRP';
        }
        // If missing required assessment, mark as INC
        else if (missingAssessments.length > 0) {
          gm = 'INC';
        } else {
          const t = totalGrade;
      ...
      // Compute REMARKS based on missing assessments or grade
      if (remarksCell) {
        let remarks = 'PASSED';
        
        // Function to abbreviate assessment names
        const abbreviateAssessment = (name) => {
          const nameLower = name.toLowerCase();
          if (nameLower.includes('prelim') && nameLower.includes('exam')) return 'NPE';
          if (nameLower.includes('midterm') && nameLower.includes('exam')) return 'NME';
          if (nameLower.includes('final') && nameLower.includes('exam')) return 'NFE';
          if (nameLower.includes('project')) return 'NO PROJECT';
        ...
        // If missing required assessments, show all missing components
        else if (missingAssessments.length > 0) {
          const abbreviated = missingAssessments.map(name => abbreviateAssessment(name));
          remarks = abbreviated.join(', ');
        } else {
          // Determine remarks based on grade mark
          try {
    ...
    init() {
      ...
      // Initialize dropped states for already-dropped students
      try { this.initializeDroppedStates(); } catch(_) {}
    ...
    /**
     * Initialize dropped states on page load - disable inputs for dropped students
     */
    initializeDroppedStates() {
      // Find all dropped checkboxes that are checked
      const droppedCheckboxes = document.querySelectorAll('input.dropped-checkbox:checked');
      droppedCheckboxes.forEach(checkbox => {
        const studentId = checkbox.getAttribute('data-student');
        const mainRow = this.tbody.querySelector(`tr[data-student-id="${studentId}"]`);
        if (mainRow) {
          this.toggleStudentInputs(mainRow, true);
        }
      });
    }

    /**
     * Toggle student input fields (disable when dropped)
     */
    toggleStudentInputs(row, isDropped) {
      const inputs = row.querySelectorAll('input.score');
      inputs.forEach(input => {
        input.disabled = isDropped;
      });
      
      // Add/remove visual indicator
      const studentCell = row.querySelector('td.sticky-left:nth-child(2)');
      if (studentCell) {
        let droppedLabel = studentCell.querySelector('.dropped-label');
        
        if (isDropped) {
          if (!droppedLabel) {
            droppedLabel = document.createElement('div');
            droppedLabel.className = 'dropped-label';
            droppedLabel.textContent = 'DROPPED STUDENT';
            droppedLabel.style.cssText = `
              background: #dc2626;
              color: white;
              padding: 2px 8px;
              border-radius: 4px;
            ...
            `;
            studentCell.appendChild(droppedLabel);
          }
